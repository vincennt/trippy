...
const HotelScroll = (props) => {
  ...
  return (
    <Hotel
      key={props.hotel.name}
      ref={ref}
      color={props.hotel._id === props.selectHotel._id ? "#9BE895" : "#dbdbdb"}
      onClick={() => {
    ...
    >
      <Link key={props.hotel._id} to={`/hotels/${city}/${props.hotel._id}`}>
        <Image src={props.src} alt={props.hotel.name} />
      </Link>
      <Dive>
      ...
      {!isFavorite(props.hotel._id) ? (
        <Button onClick={() => handleAddStorage(props.hotel._id)}>
          Add Fav
        </Button>
      ) : (
        <Button onClick={() => handleRemoveStorage(props.hotel._id)}>
          Remove Fav
        </Button>
      )}
    </Hotel>
